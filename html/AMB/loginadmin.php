...
if ($_POST) {
  ...
  if (count($errorLogin) == 0) {
    ...
    $login=$bbdd->prepare("SELECT * FROM entre_diagonales.admin WHERE password LIKE :pass");
...
<head>
  ...
  <?php include_once "estiloAdmin.php" ?>
  <title>| LOGIN |</title>
...
<body>
  <section class="container" id="form-login">
    <p class="h2 text-center text-uppercase"><strong>Ingreso administrador</strong></p>
